...
# If the release branch already exists, skip this so we don't muck
# up the various branches
if ! wrap_raw git ls-remote --exit-code --heads origin "${release_branch}"; then
    ...
    echo "Skipping repository updates because branch ${release_branch} already exists"
fi
